...
(defn- dataset-creation-success?
  [ds]
  (not (util.data-validation/dataset-errors? ds)))
  ...
  (let [repo (repo/sail-repo)
    ...
    (let [schema (db/get-release-schema repo (su/dataset-release-uri* system-uris slugs2))
      ...
      (testing "Data validation using a row-schema"
        ...
        (let [ds (util.data-validation/as-dataset (io/resource "test-inputs/revision/one-row-invalid-double.csv") {})
              row-schema (util.data-validation/make-row-schema schema)]
          (is (contains? (util.data-validation/validate-dataset ds row-schema
                                                                {:fail-fast? true})
                         :explanation))
          (is (contains? (util.data-validation/validate-dataset ds row-schema
                                                                {:fail-fast? false})
    ...
    (let [schema (db/get-release-schema repo (su/dataset-release-uri* system-uris slugs2))
          row-schema (util.data-validation/make-row-schema schema ["Year" "Double"])]
      ...
      (testing "Create a dataset with well formatted double values"
        (let [ds (-> (io/resource "test-inputs/revision/valid-double-column.csv")
                     (util.data-validation/as-dataset {:enforce-schema row-schema}))
              validation-result (util.data-validation/validate-dataset ds row-schema
                                                                       {:fail-fast? true})]
      ...
      (testing "Failure to create a dataset with incorrectly formatted double values"
        (let [csv (io/resource "test-inputs/revision/invalid-double-column.csv")]
          (is (thrown-with-msg? clojure.lang.ExceptionInfo
                                ...
                                (util.data-validation/as-dataset csv {:enforce-schema row-schema})))))
      ...
      (testing "Create dataset from one-row CSV with invalid double value"
        ...
        (let [csv (io/resource "test-inputs/revision/one-row-invalid-double.csv")]
          ...
          (let [ex (try (util.data-validation/as-dataset csv {:enforce-schema row-schema})
